...
#include <proxygen/httpserver/samples/stress_mvfst/FizzContext.h>
#include <proxygen/httpserver/samples/stress_mvfst/HQLoggerHelper.h>
#include <proxygen/httpserver/samples/stress_mvfst/InsecureVerifierDangerousDoNotUseInProduction.h>
...
void TGConnection::connectSuccess() {
  ...
  VLOG(1) << "Connection successful on " << session_->getLocalAddress().describe();
  if (nextRequest) {
    VLOG(1) << "Running queued request...";
    nextRequest.value()();
    nextRequest.clear();
  }
}
...
void TGConnection::connectError(
  ...
  LOG(INFO) << "TGConnection failed to connect, error="
